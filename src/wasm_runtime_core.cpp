...
int strcmp(const char *left, const char *right) {
  ...
  return static_cast<unsigned char>(left[i]) -
...
char *strerror(int) { return const_cast<char *>("unsupported"); }
...
void srand(unsigned int seed) { rand_state = seed == 0 ? 1 : seed; }
...
int snprintf(char *, unsigned long, const char *, ...) { return -1; }
...
static void umul64wide(uint64_t left, uint64_t right, uint64_t *high,
                       uint64_t *low) {
...
__int128 __multi3(__int128 left, __int128 right) {
  ...
  return static_cast<__int128>(
      (static_cast<unsigned __int128>(result_high) << 64) | low_product_low);
}
